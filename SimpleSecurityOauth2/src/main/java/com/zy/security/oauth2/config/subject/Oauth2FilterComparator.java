...
* @Description oauth2相关Filter的排序器
* @version 
*/
@SuppressWarnings("deprecation")
public class Oauth2FilterComparator implements Comparator<Filter> {
	private static final int STEP = 10;
	/** 集合排序的依照 */
	private Map<String, Integer> filterToOrder = new HashMap<>();
	private int order;
	
	public Oauth2FilterComparator() {
		...
		order = STEP *4;
		put(OAuth2AuthenticationProcessingFilter.class, order);
		put(ClientCredentialsTokenEndpointFilter.class, ++order);
		...
		order = STEP *9;
		put(SlowprocessingEndpointFilter.class, order);
	}
